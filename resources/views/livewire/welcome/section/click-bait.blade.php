<div>
    ...
    <div class="col-lg-6 col-md-6 col-sm-2 col-6 mb-3">
        <div class="bg-white h-100 p-2 border">
            <div class="mb-1">
                <a class="badge badge-primary text-uppercase font-weight-semi-bold p-1 mr-2" href="{{URL::signedRoute('More Details',[$bait->id])}}">News</a>
                <a class="text-body" href=""><small>{{ $bait->created_at->format('M d, Y') }}</small></a>
            </div>
            <a class="h6 m-0 text-secondary text-lowercase" href="{{URL::signedRoute('More Details',[$bait->id])}}">{{ Str::limit($bait->title, 40) }}</a>
