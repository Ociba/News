        ...
        <div class="card-header border-0">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                ...
                <div class="d-flex flex-row" style="width: 100%; max-width: 300px;">
                    ...
                    <a href="#" class="btn  btn-neutral w-100" onclick="Livewire.dispatch('openModal', { component: 'admin.forms.add-photos-on-sell' })">Add Photo</a>
                </div>
            ...
            <div class="card-body p-0">
                ...
                <table class="table align-items-center table-flush">
                    ...
                    <tbody class="list">
                        ...
                        <tr>
                            <td>{{$i + 1}}</td>
                            <td scope="row" class="text-capitalize">{{$photo->section->section_name}}</td>
                            <td scope="row">{{$photo->title}}</td>
                            <td scope="row">{{$photo->description}}</td>
                            <td><img src="{{ asset('storage/photos/'.$photo->image_without_watermark)}}" alt="{{ $photo->title }}" style="width:50px;  height:40px;"></td>
                            <td><img src="{{ asset('storage/photos/'.$photo->image_with_watermark)}}" alt="{{ $photo->title }} - with watermark" style="width:50px;  height:40px;"></td>
                            <td scope="row">{{$photo->price}}</td>
                            <td scope="row">{{$photo->license_type}}</td>
                            <td scope="row">{{$photo->download_count}}</td>
                            <td class="budget">{{$photo->status}}</td>
                            <td class="">
